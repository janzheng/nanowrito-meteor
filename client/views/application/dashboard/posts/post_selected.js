...
Template.postSelected.helpers({
  ...
  hasGoals: function() {
    if (post.options.novelType == 'free')
      return false;
    return true
  },
  novelType: function() {
    novelType = this.options.novelType;
...
Template.postSelected.events({
  ...
  'click .post-nav-delete': function(e) {
    ...
    if (confirm("Permanently Delete this Novel?")) {
...
var copying = false; // used to limit # times you can click copy so you don't overwhelm the timer
function initClipboard() {
  console.log('init clipboard')
  console.log('copying status: ' + copying)
  var copy_sel = new $('.post-nav-copy');
  console.log(copy_sel)
